...
function NavGroup({
  ...
  return (
    <div className="px-2 mb-4">
      <p className="text-[10px] font-medium text-gray-400 dark:text-gray-500 uppercase tracking-widest px-2 mb-1">
        {label}
      </p>
      {items.map((item) => {
        ...
        return (
          ...
          >
            <i
              className={`ti ${item.icon} text-base ${active ? "text-emerald-600 dark:text-emerald-400" : ""}`}
              aria-hidden="true"
            />
            {item.label}
          </Link>
...
export default function Sidebar({ mobileOpen, onClose }: SidebarProps) {
  ...
  const sidebar = (
    <aside className="w-52 min-w-[208px] h-screen flex flex-col bg-white dark:bg-gray-900 border-r border-gray-100 dark:border-gray-800">
      {/* Logo */}
      <div className="flex items-center gap-2 px-4 py-4 border-b border-gray-100 dark:border-gray-800">
        <div className="w-7 h-7 rounded-md bg-gray-900 dark:bg-gray-700 flex items-center justify-center">
          <i className="ti ti-briefcase text-emerald-400 text-sm" aria-hidden="true" />
        </div>
        <span className="text-sm font-semibold text-gray-900 dark:text-gray-50 tracking-tight">
          HuntDesk
        </span>
      </div>
      ...
      <div className="border-t border-gray-100 dark:border-gray-800 px-2 py-2">
        ...
        <div className="flex items-center gap-2 px-2 py-2 mt-1">
          <div className="w-6 h-6 rounded-full bg-emerald-100 dark:bg-emerald-900 flex items-center justify-center text-[10px] font-semibold text-emerald-700 dark:text-emerald-300">
            {initials}
          </div>
          ...
          >
            <i className={`ti ${theme === "dark" ? "ti-sun" : "ti-moon"} text-sm`} aria-hidden="true" />
          </button>
          <button
            onClick={() => signOutUser()}
            className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 transition-colors"
