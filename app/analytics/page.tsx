...
const tooltipStyle = {
  contentStyle: { border: "1px solid #f3f4f6", borderRadius: "8px", fontSize: "12px", boxShadow: "0 1px 6px rgba(0,0,0,0.06)" },
...
function PercentTick(props: Record<string, unknown>) {
  ...
  return <text x={x} y={y} dy={4} textAnchor="end" fill="#9ca3af" fontSize={11}>{payload.value}%</text>;
}

function PercentTooltip(props: Record<string, unknown>) {
  const { active, payload, label } = props as { active?: boolean; payload?: Array<{ name: string; value: number }>; label?: string };
  if (!active || !payload?.length) return null;
  return (
    <div style={{ ...tooltipStyle.contentStyle, padding: "8px 12px" }}>
      <p style={tooltipStyle.labelStyle}>{label}</p>
      {payload.map((p) => <p key={p.name} style={tooltipStyle.itemStyle}>{p.name}: {p.value}%</p>)}
    </div>
  );
}
...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    if (!user) return;
    getJobs(user.uid).then((j) => { setJobs(j); setLoading(false); });
  }, [user]);
    ...
    return (
      <div className="p-6 max-w-5xl mx-auto animate-pulse space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {[1, 2, 3, 4].map((n) => <div key={n} className="h-56 bg-gray-100 dark:bg-gray-800 rounded-xl" />)}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {totalApplied === 0 ? (
        <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-xl p-14 text-center">
          <div className="w-10 h-10 rounded-xl bg-gray-50 dark:bg-gray-800 flex items-center justify-center mx-auto mb-3">
            <i className="ti ti-chart-bar text-gray-400 dark:text-gray-500 text-xl" aria-hidden="true" />
          </div>
          <p className="text-sm text-gray-500 dark:text-gray-400">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <ChartCard title="Applications by source" subtitle="Where you're finding roles" empty={!hasSourceData}>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={sourceStats} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                ...
                <Tooltip contentStyle={tooltipStyle.contentStyle} labelStyle={tooltipStyle.labelStyle} itemStyle={tooltipStyle.itemStyle} cursor={{ fill: "#f9fafb" }} />
                <Bar dataKey="applications" name="Applications" fill="#10b981" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </ChartCard>

          <ChartCard title="Applications over time" subtitle="Last 12 weeks" empty={!hasWeeklyData}>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={weeklyData} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f9fafb" />
                <XAxis dataKey="week" tick={{ fontSize: 10, fill: "#9ca3af" }} axisLine={false} tickLine={false} interval={2} />
                <YAxis allowDecimals={false} tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={tooltipStyle.contentStyle} labelStyle={tooltipStyle.labelStyle} itemStyle={tooltipStyle.itemStyle} cursor={{ stroke: "#e5e7eb" }} />
                <Line type="monotone" dataKey="count" name="Applications" stroke="#10b981" strokeWidth={2} dot={{ r: 3, fill: "#10b981", strokeWidth: 0 }} activeDot={{ r: 5, fill: "#10b981", strokeWidth: 0 }} />
              </LineChart>
            </ResponsiveContainer>
          </ChartCard>

          <ChartCard title="Response rate by source" subtitle="% of applications that reached phone screen or interview" empty={!hasSourceData}>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={sourceStats} margin={{ top: 0, right: 0, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f9fafb" />
                <XAxis dataKey="source" tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} />
                <YAxis domain={[0, 100]} tick={<PercentTick />} axisLine={false} tickLine={false} />
          ...
          <ChartCard title="Interview-to-offer conversion" subtitle="% of interviews that resulted in an offer" empty={!hasSourceData || sourceStats.every((s) => s.interviews === 0)}>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={sourceStats} margin={{ top: 0, right: 0, left: -10, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f9fafb" />
                <XAxis dataKey="source" tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} />
